...
author: 
  - name          : "Selma Tir"
    ...
    role:
      - wrote the original draft protocol, analyzed data, reviewed and edited the manuscript
  - name          : "Rhiannon White"
    affiliation   : "1,3"
...
authornote: |
  This systematic review is funded by the Wellcome Trust (Sir Henry Wellcome Fellowship to MS 204686/Z/16/Z; Research Enrichment – Diversity & Inclusion WT 204686/Z/16/A) and Linacre College (Biomedical Sciences Junior Research Fellowship to MS).
abstract: |
...
bibliography      : Demographics Review References.bib
...
r_refs("Demographics Review References.bib")
```
...
data.original <- read_excel("scoring.xlsx")
```
...
figure1A <- ggplot(data.included, aes(x=article_year)) +
...
figure2A <- ggplot(data.funding, aes(x=article_year, y=per)) +
...
figure4B <- ggplot(data=age.corr.journal, 
                   aes(x=variable, y=article_journal, fill=value)) +
  geom_tile() +
  theme_apa() +
  ylab(label="Journal") +
  xlab("Age variable") +
  scale_fill_viridis_c(end=0.6, direction=-1) +
  theme(axis.title=element_text(face="bold", size=12), 
        axis.text=element_text(colour="black", face="bold"), 
        axis.line=element_line(size=0.8), axis.ticks=element_line(size=0.8),
        legend.position="none", legend.margin=margin(0,0,0,0),
        legend.box.margin=margin(-10,0,-10,-10), legend.title=element_blank())
...
figure4C <- ggplot(data=data.age.mean, aes(x=article_year, y=age_reported_mean_value)) +
  ...
  #geom_text(x=1988, y=10, size=3.3,
            #label="Mean age = 281.5 - 0.1 * Year of publication") + # edit manually
  #geom_text(x=1988, y=7, size=3.1, label="R-Sq = 0.3%") + # edit manually
  theme_apa() +
  ylab("Mean age (years)") +
  xlab("Year of publication") +
...
figure6 <- ggplot(data.demo.reported, aes(x=article_year, y=per, color=factor(variable))) +
  geom_point(size=2.5) +
...
```{r, fig.cap="Inclusion and exclusion of articles. A. Number of articles included by journal and year of publication. B. Proportion of articles included and excluded by reason for exclusion and year of publication.", warning=FALSE, message=FALSE}
figure1B
```
