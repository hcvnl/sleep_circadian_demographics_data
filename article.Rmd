...
setwd("~/Sleep Review/sleep_circadian_demographics_data-main")
...
In this article, we addressed the question of how representative, diverse and inclusive published research articles in sleep and chronobiology are. We analyzed a sample of >1300 articles published in sleep research and chronobiology journals between 1979 and 2019 for its inclusion, reporting and analysis of study population characteristics, including age, sex, gender, ethnicity, level of education, socio-economic status, and profession.  We found that while >90% of studies reported age or sex, all other variables were reported in <25% of cases, with the frequency of reporting changing over time. We identify opportunities for improving the reporting of demographic variables.

# Research Agenda

Future research needs to:
1. Establish schemas for reporting demographic variables in a harmonized way across geographical and cultural contexts;
2. Identify gaps in the sleep and chronobiology literature with respect to understudied populations;
3. Understand the extent to which research practices allow for the inclusion of diverse populations in all stages of the research cycle, and how this can exacerbate health inequities.

# Practice Points

1. Published studies on circadian and sleep physiology should be carefully examined.
2. Reporting of demographic variables should be done carefully and systematically.
3. Inclusion and diversity of different populations across the field needs to be ensured.
...
The extent to which a scientific field's findings are generalizable is a function of the representativeness of a given study sample, and the degree to which findings differ between, e.g., different sexes and ethnicities. In the domain of sex, a recent study reviewed the reporting and analysis of sex in biological sciences research [@woitowich_10-year_2020]. The authors found that while sex inclusion has significantly increased over the past 10 years [@beery_sex_2011], sex-based analysis has not improved, despite recent policies and funder mandates [@clayton_policy_2014]. Indeed, in 2015 the United States' National Institutes of Health (NIH) issued a notice requiring grant holders to factor sex in the design, analysis and reporting of vertebrate and human studies, or to provide subtsantial justification for the study of a single sex [@nih-2015notice]. The term "gender data gap" has recently been introduced, demonstrating that women have historically been excluded from biomedical research [@criado-perez_invisible_2020]. Similarly, the lack of diversity in biomedical and clinical research, and understudy of minorities in the presence of existing health inequities exacerbates these inequities [@oh_diversity_2015]. Indeed, a recent review of contemporary dementia research reported a lack of demographic, racial, and geographic diversity [@Mooldijk2021].
...
All included articles were reviewed for eligibility and coded by RW. The reporting of sample size, age, sex, gender, ethnicity, level of education, socio-economic status, and profession of the study population was scored binarily (0 = not reported, 1 = reported), and any reported aggregate summary statistics for these variables were recorded (e.g., mean, median, etc.). Sample size referred to the total number of participants for a given study. If an article reported multiple studies, then it was analyzed for each of its individual study. Age was analyzed when recorded in days, weeks or years. Sex referred to biological sex, while gender referred to the social construction of sex. Since the language and system describing ethnicity, level of education, socio-economic status and profession differ between countries and individuals, all reporting was taken into account as long as there was a clear indication of what the variable represented. Additionally, funding source, geographical location and clinical focus of the article were examined, as well as whether data were analyzed by including any of the demographic variables as covariates. Data were coded in an Excel Spreadsheet and analyzed in R Studio (version 4.0.5).
...
figure1 <- ggplot(data.included, aes(x=article_year)) +
  ...
  labs(fill="Journal") +
  ggtitle("Included and analyzed articles by journal and year") +
...
data.eligibility <- data.eligibility %>%
  plyr::count(vars=c("eligibility", "article_year")) %>%
  ungroup()
...
data.eligibility <- data.eligibility %>%
  ...
  mutate(per=round(100*freq/total,3))
...
We pre-registered our protocol (specified using the PRISMA-P template [@shamseer_preferred_2015; @prisma-p_group_preferred_2015]) on the Open Science Framework (https://osf.io/cu3we/). For thorough details on the screening and analysis methods, see the protocol.

## Materials, data and code availability

All data underlying this manuscript are available on a public GitHub repository (https://github.com/hcvnl/sleep_circadian_demographics_data). The article was written in R [@R-base] using RMarkdown and papaja [@R-papaja], employing a series of additional R packages [@R-corrplot2017; @R-dplyr; @R-ggplot2; @R-gridExtra; @R-knitr; @R-latticeExtra; @R-mosaic; @R-mosaicData; @R-pracma; @R-readxl; @R-reshape2; @R-stringr; @R-tidyr] and is fully reproducible.
...
data.funding <- data.funding %>%
  plyr::count(vars=c("funding", "article_year")) %>%
  ungroup()
...
data.country <- data.included %>%
  filter(!grepl(",", study_country_values)) %>%
  plyr::count(vars=c("study_country_values", "article_year")) %>%
  ungroup()
...
data.country <- aggregate(cbind(freq) ~ study_country_values + article_year, data=data.country, FUN=sum)
...
#data.age.year$article_year <- as.factor(data.age.year$article_year)
...
figure6A <- ggplot(data=age.corr.year, aes(x=variable, y=article_year, fill=value)) +
  geom_tile() +
  theme_apa() +
...
figure6B <- ggplot(data=age.corr.journal, 
...
figure7 <- ggplot(data=data.age.mean, aes(x=article_year, y=age_reported_mean_value)) +
...
data.sex <- data.sex %>%
  plyr::count(vars=c("report", "article_year")) %>%
  ungroup()
...
figure8 <- ggplot(data.sex, aes(x=article_year, y=per)) +
...
data.gender <- data %>% 
  filter(gender_reported_yn==1)
```

```{r Other demographics, message=FALSE, warning=FALSE, include=FALSE}
...
data.demo.education <- data.demo %>%
  plyr::count(vars=c("education", "article_year")) %>%
  ungroup()
data.demo.ethnicity <- data.demo %>%
  plyr::count(vars=c("ethnicity", "article_year")) %>%
  ungroup()
data.demo.profession <- data.demo %>%
  plyr::count(vars=c("profession", "article_year")) %>%
  ungroup()
data.demo.ses <- data.demo %>%
  plyr::count(vars=c("ses", "article_year")) %>%
  ungroup()
...
figure9 <- ggplot(data.demo.reported, aes(x=article_year, y=per, color=factor(variable))) +
  geom_point(size=1.2) +
  geom_line() +
  theme_apa() +
  xlab("Year of publication") +
...
figure10A <- ggplot(data=data.education, aes(x=education_reported_numcategory)) +
  ...
  scale_y_continuous(limits=c(0,0.5), expand=c(0,0), breaks=c(0,0.2,0.4), labels=c("0%","20%","40%")) +
  theme(axis.title=element_text(face="bold", size=11), 
        axis.text=element_text(colour="black", face="bold", size=10),
        axis.line=element_line(size=0.8), axis.ticks=element_line(size=0.8)) 

# compute the education category names into a list
education.category.list <- tolower(data.education$education_reported_categories)

# check the category combination that is most frequent 
table(education.category.list)

# compute the number of articles reporting education as a mean
data.education.mean <- data.education %>%
  filter(education_reported_mean_yn==1)
percent.education.mean <- round(nrow(data.education.mean)/nrow(data.education) * 100, 2)

data.education.mean$education_reported_mean <- as.numeric(data.education.mean$education_reported_mean)
data.education.mean <- data.education.mean %>%
  filter(!is.na(education_reported_mean))
mean.education <- mean(data.education.mean$education_reported_mean)
...
# Compute percentage of articles where people can select multiple ethnicities out of those that report ethnicity
data.ethnicity.multiple <- data %>%
  filter(ethnicity_multiple_categories_yn == 1)

percent.ethnicity.multiple <- round(nrow(data.ethnicity.multiple)/nrow(data.ethnicity) * 100, 2)
...
figure10B <- ggplot(data=data.ethnicity, aes(x=ethnicity_reported_numcategory)) +
  geom_histogram(binwidth=1, aes(y=..density..), colour="black", fill="#2D708EFF") +
...
# compute the ethnicity category names into a list
ethnicity.category.list <- tolower(data.ethnicity$ethnicity_reported_categories)

# check the category combination that is most frequent 
table(ethnicity.category.list)

# split the category combinations and convert to lower case letters
ethnicity.category.list <- as.list(strsplit(ethnicity.category.list, ","))
ethnicity.category.list <- unlist(ethnicity.category.list)

# count the number of occurences for each category
table(ethnicity.category.list)

# count the number of occurrences for "white"/"caucasian", "african american"/"black", and "other"
n.ethnicity.white <- sum(str_count(ethnicity.category.list, "white"),str_count(ethnicity.category.list, "caucasian"),str_count(ethnicity.category.list, "non hispanic white"),str_count(ethnicity.category.list, "white caucasian"),str_count(ethnicity.category.list, "white american"))
n.ethnicity.black <- sum(str_count(ethnicity.category.list, "black"),str_count(ethnicity.category.list, "african american"),str_count(ethnicity.category.list, "african-american"),str_count(ethnicity.category.list, "non hispanic black"))
n.ethnicity.other <- sum(str_count(ethnicity.category.list, "other"))

# compute percentage of articles where white vs black is reported as an ethnicity
percent.ethnicity.white <- round(n.ethnicity.white/nrow(data.ethnicity) * 100, 2)
percent.ethnicity.black <- round(n.ethnicity.black/nrow(data.ethnicity) * 100, 2)
percent.ethnicity.other <- round(n.ethnicity.other/nrow(data.ethnicity) * 100, 2)
...
figure10C <- ggplot(data=data.profession, aes(x=profession_reported_numcategory)) +
  geom_histogram(binwidth=1, aes(y=..density..), colour="black", fill="#2D708EFF") +
...
# compute the profession category names into a list
profession.category.list <- tolower(data.profession$profession_reported_categories)

# check the category combination that is most frequent 
table(profession.category.list)

# split the category combinations
profession.category.list <- as.list(strsplit(profession.category.list, ","))
profession.category.list <- unlist(profession.category.list)

# count the number of occurences for each category
table(profession.category.list)

# count the number of occurrences for "unemployed"
n.profession.unemployed <- sum(str_count(profession.category.list, "unemployed"))

# compute percentage of articles where this is reported
percent.profession.unemployed <- round(n.profession.unemployed/nrow(data.profession) * 100, 2)
...
figure10D <- ggplot(data=data.ses, aes(x=ses_reported_numcategory)) +
  geom_histogram(binwidth=1, aes(y=..density..), colour="black", fill="#2D708EFF") +
...
# compute the ses category names into a list
ses.category.list <- tolower(data.ses$ses_reported_categories)

# check the category combination that is most frequent 
table(ses.category.list)

# compute percentage of articles where ses is based on income with numbers
percent.ses.income <- 100 - round(21/nrow(data.ses) * 100, 2)
```
...
figure11 <- ggplot(data.analysis, aes(x=article_year, y=per)) +
...
Out of `r as.numeric(nrow(data.original))` identified and pre-screened articles, we included and extracted data from `r as.numeric(nrow(data.included))` (`r round(percent.included)`%). The distribution of years in which the articles were published is non-uniform and the proportion of included and extracted data is greater for more recent articles (Fig. 1). In addition, the representation of journals in the final list of articles is also non-uniform, as the included journals were not all available from the start date of our data collection (1979 onwards).
  
```{r, fig.cap="Included and analyzed articles by year and journal of publication. Recent articles are more widely represented, reflecting an overall increase in scientific output.", warning=FALSE, message=FALSE}
figure1
```
...
```{r, fig.cap="Percentage of included and excluded articles by year of publication, with reasons for exclusion.", warning=FALSE, message=FALSE}
figure2
```
...
## Geographical location
  ...
  `r round(100-percent.country.multiple)`% of articles were conducted in a single country. The geographical location of the study was explicitly reported in `r round(percent.country.reported)`% of studies. The country of study was inferred for the rest of the sampled articles. Inference was primarily based on the first author's affiliation. Overall, `r nrow(country.single)` countries were represented. Figure 4 shows the distribution of study location across time with the eight most represented countries. Across all years, the US is the most represented country and only three studies were primarily conducted in Africa (Nigeria, Senegal and Tunisia).
...
## Age
  ...
  `r round(percent.age)`% of articles reported a variable describing age, such as the mean, standard deviation, median, minimum, maximum and interquartile range. Overall, the median and interquartile range were the least reported variables for sex (Fig. 6). The minimum and maximum age were the most employed variables in 1979, while their reporting decreased throughout the years compared to other variables. On another hand, the standard deviation of the mean was increasingly reported throughout the years. Trends for specific journals were also observed, such as the substantial use of the min/max variables in *Journal of Pineal Research*, and the lack of in *Journal of Clinical Sleep Medicine*.
  
```{r, warning=FALSE, message=FALSE}
figure6A
```

```{r, fig.cap="Reporting of various age variables by year (top) and journal (bottom) of publication. SD = standard deviation of the mean, IQR = interquartile range.", warning=FALSE, message=FALSE}
figure6B
```

  Overall, the average mean age of the study populations was `r round(mean(data.age.mean$age_reported_mean_value))` years old. We examined the extent to which the mean age differed across studies as a function of publication year (Fig. 7), and found that the mean age is much more widely varied in later years, likely reflecting the extent of considering study samples that are more diversified in age.
...
## Sex & Gender
  ...
  Sex was reported in `r round(percent.sex)`% of the studies. In Figure 8, we show the proportion of studies that recruited male subjects, female subjects, both sexes or did not specify the sex of the participants. Overall, `r round(percent.sex.male)`% of the studies reporting sex only recruited male participants, while `r round(percent.sex.female)`% only employed females. Out of the studies focusing on a single sex, `r round(percent.sex.male.focus)`% of the male studies focused on a sex dependent feature, while `r round(percent.sex.female.focus)`% of the female studies did. `r round(percent.sex.age)`% of studies reported age by sex. 
  
  Gender was reported in only one article, reflecting the lack of inclusion for gender identity. In the reporting gender, participants were described as male, female or transgender. 
...
##  Ethnicity, education, profession and socio-economic status
  ...
  We examined the reporting of further demographic variables, including ethnicity, education, socio-economic status and profession. These demographics variables were reported in `r round(percent.education)`% of studies for education, `r round(percent.ethnicity)`% for ethnicity, `r round(percent.ses)`% for socio-economic status and `r round(percent.profession)`% for profession. Figure 9 shows the distribution of this reporting across the years. Qualitatively, there is a clear increase in the reporting of additional demographic variables over time, with ethnicity being the most reported demographic variable across articles in more recent years. Participants could choose multiple categories for ethnicity in `r round(percent.ethnicity.multiple)`% of the studies reporting this variable. The mean number of years of education was reported in `r round(percent.education.mean)`% of articles, and was `r round(mean.education)` years of study overall.

```{r, fig.cap="Reporting of education, ethnicity, profession and socio-economic status by year of publication.", warning=FALSE, message=FALSE}
figure9
```
  
  We furthermore examined the number of categories included for each of these demographic variables. In Figure 10, we show the number of categories reported for each variable among those articles that included it. On overage, we find that the coding scheme for each of these comprised between three and four variables. Education was reported as a range of years of study, degree level, or a subjective measure such as "low, medium, high". The most common categories for ethnicity were "white/caucasian", "black/african american" and "other", and were mentioned in `r round(percent.ethnicity.white)`%, `r round(percent.ethnicity.black)`% and `r round(percent.ethnicity.other)`% of studies respectively. The most common category for profession was "unemployed", mentionned in `r round(percent.profession.unemployed)`% of studies, along with a variety of specific jobs, such as attorney, farmer and astronaut. In `r round(percent.ses.income)`% of studies reporting socio-economic status, the categories of SES were based on range of income. The rest of the articles reported SES in subjective measures, such as "high, medium, low". 

```{r, fig.cap="Distribution of the number of categories reported for education, ethnicity, profession and socio-economic status (SES). The yellow vertical line corresponds to the median.", warning=FALSE, message=FALSE}
grid.arrange(figure10A, figure10B, figure10C, figure10D, nrow=2, top="Number of reported categories")
```


## Study focus

  We considered whether a given article reported on a specific, pre-defined group of people. We found that `r round(percent.focus.sex)`% of articles focused on a sex dependent feature, while `r round(percent.focus.clinical)`% investigated a clinical feature. `r round(percent.focus.twins)`% of studies focused on twins, `r round(percent.focus.pregnant)`% on pregnant women, `r round(percent.focus.shift)`% on shift workers and `r round(percent.focus.students)`% on university students. 


## Analysis disaggregation

  We examined the extent to which articles reported subgroup analyses of the data based on one or more of the reported demographic variables. Over time (Fig. 11), we see a distinct evolution of the extent to which subgroup analyses of the study sample were performed. The most common subgroup analyses involve disaggregating by sex, age, or both.
...
Here, we considered the inclusion and reporting of `r as.numeric(nrow(data.included))` articles in chronobiology and sleep research, sampled from eight journals. Articles from recent years were more widely included, with articles originating in the US representing the largest group. While studies conducted in Western countries in North America and central Europe were predominant, those in Africa only represented 0.3% of all screened articles, underlining a gap in geographical inclusion. Over time, we found an increase in the range of sample sizes, as well as a wider spread of average ages included in the studies. We also observed an increase in the reporting of demographic variables (ethnicity, education, professional and socioeconomic status), with ethnicity being the most-reported variable in recent years. A wide variety of reporting strategies, which involve both objective and subjective measures, was also observed. 

## Taking an inventory of represented study samples reveals the representativeness of our collective knowledge

The ability to generalize findings from the scientific literature to wide and diverse populations hinges upon the representativeness of the study sample with respect to demographic categories. The question to what extent the composition of a given study sample can make the generalizability of findings difficult or impossible has received attention in the field of psychology, where many articles published in prominent journals reflected participants from WEIRD (Western, Educated, Industrialized, Rich, and Democratic) contexts [@henrich_weirdest_2010; @muthukrishna_beyond_2020]. In other fields, analyses similar to the one in the present review have been published [@obryant_reporting_2004; @sifers_reporting_2002; @jones_reporting_2020;@Mooldijk2021], but to our knowledge, this review represents a first look at the inclusion, reporting and analysis of participant demographics in chronobiology and sleep research.
